...
import com.akiteam.akiloader.event.NeoForgeEventBridge;
...
@Mod(AkiLoader.MODID)
public class AkiLoader {
    public static final String MODID = "akiloader";
    public static final Logger LOGGER = LogUtils.getLogger();

    /** 插件默认存放目录（位于游戏运行目录下）。 */
    public static final String PLUGINS_DIR = "akiloader";

    /**
     * AkiLoader 当前对外开放的 API 版本。
     * <p>
    ...
    /** NeoForge → AkiLoader 事件桥接器，把游戏事件转发为插件事件。 */
    private final NeoForgeEventBridge eventBridge;

    // The constructor is the first code run when the mod is loaded.
    public AkiLoader(IEventBus modEventBus, ModContainer modContainer) {
        LOGGER.info("AkiLoader has been initialized!");
     ...
     *
     * @return 由 {@code AkiLoader} 构造时创建的插件管理器（正常情况下不会为 {@code null}）
     */
