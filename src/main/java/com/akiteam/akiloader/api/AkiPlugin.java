package com.akiteam.akiloader.api;

import com.akiteam.akiloader.command.CommandRegistry;
import com.akiteam.akiloader.config.PluginConfig;
import com.akiteam.akiloader.event.EventBus;
import com.akiteam.akiloader.permission.Permissible;
import com.akiteam.akiloader.persistence.AkiAdapterContext;
import com.akiteam.akiloader.persistence.AkiPersistentDataHolder;
 ...
 * <p>
 * 事件监听：插件主类上直接标注 {@code @AkiEventHandler} 的方法会被自动注册到事件总线；
...
public interface AkiPlugin {
     ...
     * 因此主类上直接写 {@code @AkiEventHandler} 方法即可生效，无需覆盖本方法。
     ...
     * （对应 {@code run/akiloader/<插件名>/config.yml}）并调用本方法交给插件。
