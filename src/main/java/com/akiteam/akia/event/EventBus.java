...
/**
 * Akia 的插件事件总线（参考 Paper {@code SimplePluginManager} 的事件分发模型）。
 * <p>
 * <b>注册</b>：{@link #registerEvents(AkiPlugin, Object)} 扫描监听器对象上所有带
 * {@link EventHandler} 的方法，按事件类型分组、按优先级排序后存储。
 * <p>
 * <b>分发</b>：{@link #fireEvent(Event)} 按 事件类型 → 优先级(LOWEST→NORMAL→HIGHEST→MONITOR)
 * 依次反射调用监听器。取消语义：
 * <ul>
...
public class EventBus {
     ...
     * @param method        带 {@link EventHandler} 注解的回调方法
     * @param priority      优先级
    ...
    /**
     * 统计某插件当前注册的监听器方法总数（按 {@code @EventHandler} 方法计）。
     * 供诊断命令统计每个插件的事件监听数量。
     *
     * @param plugin 目标插件
     * @return 该插件注册的监听器方法数
    ...
    /**
     * 注册一个监听器对象中所有带 {@link EventHandler} 的方法。
     * <p>
     * 会沿类的继承链向上收集方法。无效的处理器（参数不是单个 {@link Event}、
    ...
    public void registerEvents(AkiPlugin plugin, Object listener) {
        ...
        for (Method method : collectEventMethods(listener.getClass())) {
            ...
            if (method.getReturnType() != void.class
                ...
                LOGGER.warn("Skipping invalid @EventHandler '{}': needs one {} parameter and void return.",
                        method, Event.class.getSimpleName());
                continue;
            }
