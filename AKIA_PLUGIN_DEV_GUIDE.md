...
> 面向：从零理解 Akia 插件模型、并据此把 Paper 的 PlaceholderAPI(PAPI) 移植到 Akia 的开发者 / AI。
...
插件（Plugin）是一个独立的 JAR 文件，实现 `com.akiteam.akia.api.AkiPlugin` 接口，被 Akia 在游戏启动时扫入并管理。每个插件拥有独立的类加载器（ClassLoader），插件与插件之间默认互相不可见（类隔离）。

插件有明确的生命周期，由 Akia 驱动，顺序固定：
...
Akia 扫描运行目录下的插件目录（运行时为 `run/akia/`），读取每个 `.jar` 里的 `plugin.json` 元数据，解析出插件名、主类、依赖等信息并实例化主类。
...
Akia 不要求插件从某个基类继承，而是用一组默认空实现的 `registerXXX` 钩子在主类上"注入能力"。每个钩子都在加载阶段被 Akia 调用，插件把对应的能力对象存进自己的字段即可使用。
...
插件 JAR 的根目录必须放一个 `plugin.json`。它由 Gson 解析，是 Akia 认识插件的唯一入口。
...
用 `@EventHandler` 注解监听内建插件事件。插件主类自身会被自动注册为监听器，无需手动；若要注册其他对象作监听器，在 `registerEvents` 里调用 `bus.registerEvents(this, myListener)`。

注解属性：`priority`（EventPriority：LOWEST/LOW/NORMAL/HIGH/MONITOR，默认 NORMAL）、`ignoreCancelled`（默认 false）。

事件：目前内建 `PlayerJoinEvent`（进服事件，有 `getPlayer()` 与 `getPlayerName()`）。取消语义：事件若实现 `Cancellable` 且已取消则停止传播。
...
Akia 命令基于 Brigadier，但封装成"扁平"模型：命令名 + 可选单个"贪心字符串参数"。\*\*Akia 目前不建子命令树，所有子命令都要用"一个贪心参数 + 在 lambda 里按第一个单词 switch 派发"实现；Tab 补全则通过查看已输入文本动态返回候选。\*\*这是最有用的命令写法，务必掌握。
...
public void registerCommands(CommandRegistry registry) {
    registry.register("hello", ctx -> {
        ctx.getSource().sendSuccess(
            () -> Component.literal("Hello from Akia!"), false);
        return 1;
    });
...
- `suggestion` 是函数式接口 `com.akiteam.akia.command.AkiCommandSuggestion`，签名 `List<String> suggest(CommandContext<CommandSourceStack> ctx, String remainingInput)`。
...
`/akia reload` 会先清除所有插件命令节点，再重新注册，因此新打包的命令无需重启游戏即生效。
...
在 `registerConfig(PluginConfig)` 里保存。对应磁盘 `run/akia/<插件名>/config.yml`；文件不存在时从插件 JAR 根目录的默认 `config.yml` 复制。
...
插件卸载/重载时，Akia 自动 `unregisterAll(plugin)`，无需手动清理。
...
问题：默认每个插件用独立 ClassLoader 加载自己的类。如果消费者把"服务接口"也打进自己 jar，Akia 父加载器（Akia 自身）里又没有这个接口，那么提供者和消费者各自加载出**两份不同的 Class 对象**。而 ServiceRegistry 以 `Class` 为 key，消费者用自己那份 Class 去查提供者注册的服务，永远查不到（这是最常见的移植坑，务必注意）。

解决办法：提供者把 API 接口所在的**包**声明到 `exports`，Akia 记录"包名 → 提供者 ClassLoader"的导出索引。之后任何插件（包括消费者）要加载该包下的类时，会被**父优先委派**给提供者加载器，从而保证同一个包只产生一份 Class。
...
// 它由 Akia 通过 exports 委派到 GreetingProvider 的加载器加载。
...
提供一个示例插件（如 ShopConsumer），建议在 Akia 仓库旁的独立目录组织源码：
...
shopconsumer/
  ...
  build.bat            （或 .sh，见下）
  out/                  （javac 输出）
  ShopConsumer.jar      （打包产物，拷到 run/akia/）
```
...
编译需要 classpath 包含 Akia 主类、Minecraft 与 NeoForge 的类。**建议把 classpath 写成一个固定的文件列表**，或用 Gradle 的 compileClasspath 输出（`gradlew dependencies` 或让脚本 Dump 出 classpath）。最简方案：把 Akia 主类 output + NeoForge merged jar + Brigadier jar 加入 `-cp`。

一个可直接用的 Windows 批处理示例（Akia 输出 + 用 `dir /s` 收集其 libs jar，失败容忍）：

```bat
@echo off
set ROOT=E:\mc\JavaLearn\Akia\MDK-1.21.1-ModDevGradle-main
rem 最简：Akia 主类 output
...
cd ..
copy /y ShopConsumer.jar %ROOT%\run\akia\ShopConsumer.jar
```
...
- 热重载（改代码重打包后，无需重启游戏）：OP 权限下在聊天框执行 `/akia reload`，观察 `enabled ... failed` 统计。
...
Akia 自带一组 OP 命令（权限等级 2），强烈建议开发时使用：

```text
/akia list                    列出插件及状态/服务数/监听器数/任务数
/akia info <插件名>            单插件完整诊断（含 exports、注册的服务），Tab 可补全插件名
/akia dump                    把完整报告导出为 run/akia/dump/dump-<时间戳>.json
/akia reload                  热重载全部插件
```

**依赖解析失败排查**：如果插件因依赖缺失没加载，`/akia list` 不会显示它，`/akia info` 也查不到。这时要去看**日志**里 `Could not load plugin ... missing required dependency`（强依赖缺失）或 `circular dependency or unsatisfiable dependency chain`（循环依赖）的报错，据此补全 `depend` 或调整依赖声明。

***

## 7. 写给 PAPI 移植者的映射要点

PlaceholderAPI 移植到 Akia 时的对应关系：

- PAPI 是一个提供占位符服务、并让其他插件注册/使用占位符的插件 → 本质就是 Akia 的 ServiceRegistry 上的一个服务。
...
- 每服务器/每玩家配置 → `PluginConfig`（路径 `run/akia/<插件名>/config.yml`）。
...
4. 编译（classpath 含 Akia build/classes + game/neoforge 类）、打包（plugin.json 在 JAR 根）。
5. 放入 `run/akia/`，`gradlew runClient` 进世界；用 `/akia list` + `/akia info <名>` 核对，或 `/akia dump` 导出报告排查。
6. 改代码后重打包，`/akia reload` 热加载。
